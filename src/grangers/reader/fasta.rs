...
impl SeqInfo {
    pub fn new(seqnames: Vec<String>, seqlengths: Option<Vec<usize>>, is_circular: Option<Vec<bool>>, genome: Option<String>, extra: Option<HashMap<String, Vec<String>>>) -> anyhow::Result<SeqInfo> {
        ...
        if let Some(hm) = &extra {
            for (k,v) in hm.iter() {
                if equal_length(&seqnames, &v) {
                    bail!("seqnames and {} have different length; Could not create SeqInfo", k)
                }
            }
        }
    ...
    pub fn from_fasta<T: AsRef<Path>>(file_path: T) -> anyhow::Result<SeqInfo> {
        ...
        let si = SeqInfo::new(seqnames, Some(seqlengths), None, Some(file_path.as_ref().to_string_lossy().to_string()), None);
        si
    }
...
fn _get_chromsize<T: BufRead>(rdr: &mut fasta::Reader<T>) -> anyhow::Result<(Vec<String>, Vec<usize>)> {
    ...
    for result in rdr.records() {
        let record = result?;
        seqnames.push(record.name().split_once(' ').unwrap_or((record.name(),"")).0.to_string());
